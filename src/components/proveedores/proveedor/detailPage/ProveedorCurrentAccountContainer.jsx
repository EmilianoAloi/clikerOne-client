  ...
  useEffect(() => {
    ...
    const facturas = (invoices || []).map((f) => {
      ...
      return {
        ...
        comprobante: f.numero_factura
          ...
          : `#${f.id_factura}`,
        razon_social:
          f.razon_social_proveedor || currentProveedor.nombre || "-",
    ...
    const pagos = (payments || [])
      .filter(
        (p) =>
          p.estado_logico === 1 &&
          p.id_proveedor === currentProveedor.id_proveedor
      )
      .map((p) => {
    ...
    <section className="rounded-lg border shadow-sm mx-auto px-6 py-4 w-full space-y-6">
      ...
      <div className="flex justify-between items-end bg-muted/30 rounded-lg p-3">
        ...
        <div className="flex gap-2">
          ...
          <Link
            to={`/proveedores/${currentProveedor.id_proveedor}/pagos/nuevopago`}
          >
            <Button
              variant="outline"
              className="text-sm font-semibold rounded-sm cursor-pointer px-4 py-2"
          ...
          <Link
            to={`/proveedores/${currentProveedor.id_proveedor}/notacredito/nuevanotacredito`}
          >
            <Button
              variant="outline"
              className="text-sm font-semibold rounded-sm cursor-pointer px-4 py-2"
          ...
          <Link
            to={`/proveedores/${currentProveedor.id_proveedor}/notadebito/nuevanotadebito`}
          >
            <Button
              variant="outline"
              className="text-sm font-semibold rounded-sm cursor-pointer px-4 py-2"
